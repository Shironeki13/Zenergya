...
import { getClient, getSitesByClient, createSite, updateSite, deleteSite } from '@/services/firestore';
...
export default function ClientDetailPage() {
  ...
  const loadClientData = useCallback(async () => {
    try {
      ...
      const sitesData = await getSitesByClient(id);
      setSites(sitesData);
    } catch (error) {
  ...
  const handleSubmitSite = async (e: React.FormEvent) => {
    ...
    try {
      if (editingSite) {
        await updateSite(editingSite.id, { name: siteName, address: siteAddress });
        toast({ title: "Site mis à jour", description: "Le site a été mis à jour avec succès." });
      } else {
        await createSite({ name: siteName, address: siteAddress, clientId: id });
              ...
              {sites.length > 0 ? (
                ...
                <TableRow>
                  <TableCell colSpan={3} className="text-center">
                    Aucun site n'a encore été créé pour ce client.
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
