...
const extractContractInfoFlow = ai.defineFlow(
  ...
  async ({ documentDataUri, activities, prompt, typologies, terms, schedules }) => {
    ...
    if (output) {
      // Ensure activityIds are populated from activitiesDetails if activitiesDetails are present but activityIds are not.
      if ((!output.activityIds || output.activityIds.length === 0) && output.activitiesDetails && output.activitiesDetails.length > 0) {
        output.activityIds = output.activitiesDetails.map(a => a.activityId);
      }
    }
    return output!;
  }
