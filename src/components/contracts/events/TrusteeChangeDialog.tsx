...
import { createTrusteeChange } from "@/services/firestore";
import { useToast } from "@/hooks/use-toast";
import { Loader2 } from "lucide-react";
import { useData } from "@/context/data-context";

const formSchema = z.object({
    newRepresentative: z.string().min(1, "Nouveau syndic requis"),
    contactEmail: z.string().email("Email invalide").optional().or(z.literal('')),
    effectiveDate: z.string().min(1, "Date requise"),
});

interface TrusteeChangeDialogProps {
    contractId?: string;
    open: boolean;
...
export function TrusteeChangeDialog({ contractId, open, onOpenChange, onSuccess }: TrusteeChangeDialogProps) {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        try {
            await createTrusteeChange({
                ...
                createdBy: "System",
                currentRepresentative: "Ancien Syndic", // TODO: Fetch current
            });
            toast({ title: "Changement de syndic enregistré", description: "Le changement a été ajouté avec succès." });
            form.reset();
            onSuccess();
            onOpenChange(false);
        } catch (error) {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    <DialogTitle>Changement de Syndic</DialogTitle>
                </DialogHeader>
                    ...
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel>Nouveau Syndic</FormLabel>
                                        <FormControl>
                                            <Input placeholder="Nom du syndic" {...field} />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <Input type="email" placeholder="contact@syndic.com" {...field} />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}
